...
function TileArea() {
  ...
  for (var i = 0; i < tileOrder.length; i++) {
    tiles.push(
      <div
        key={i}
        onClick={actionsDisabled ? null : onClickTile}
        onMouseDown={actionsDisabled ? null : onClickTile}
        onTouchStart={actionsDisabled ? null : onClickTile}
        className={`tile${isUpperCase(tileOrder[i]) ? ' used-tile' : ''}`}
      >
        {isUpperCase(tileOrder[i]) ? '' : tileOrder[i]}
      </div>
    );
  }
